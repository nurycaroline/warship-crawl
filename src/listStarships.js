...
const fs = require('fs');
const LINKS = require('./starshipsByAffiliations.json')

async function run() {
	const browser = await Puppeteer.launch()
	const linksStarships = [].concat(...LINKS.map(l => l.starshipsLinks))

	const starships = await Promise.all(
		await linksStarships.map(async (link) => {
				...
				const formatValueDiv = (data) => {
					return (
						aside
							?.querySelector(data)
							?.querySelector('div')
							?.innerText?.replace(/\[\d+\]/g, '').replace('[source?]', '') || null
					)
				}
	...
	fs.writeFileSync('./starshipsFull.json', data);
}
